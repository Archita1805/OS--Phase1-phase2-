...
void OS::Write()
{
    ...
    pcb.TTC++; // Increment Total Time Counter by 2 for Write operation
...
void OS::Load_register()
{
    ...
    cout << "Loaded from M[" << address << "] to R: " << std::string(R, 4) << endl;
}
...
void OS::Store_register()
{
    ...
    cout << "Stored from R to M[" << address << "]: " << M[address][0] << M[address][1] << M[address][2] << M[address][3] << endl;
}
...
void OS::addressMap(int VA)
{
    ...
    // PTE refers to the block nunmber stored in that page frame
    int PTE;

    PTE = (M[PTR + pageNumber][2] - '0') * 10 + (M[PTR + pageNumber][3] - '0');
...
void OS::Start_User_Pg()
{
    ...
    while (IC < 100)
    {
        ...
        // Cheak if memory loaction is blank
        ...
        if (!((IR[0] == 'G' && IR[1] == 'D') || (IR[0] == 'P' && IR[1] == 'D') || (IR[0] == 'L' && IR[1] == 'R') || (IR[0] == 'S' && IR[1] == 'R') ||
              (IR[0] == 'C' && IR[1] == 'R') || (IR[0] == 'B' && IR[1] == 'T') || IR[0] == 'H'))
        {
        ...
        IC++;
        addressMap(IC); // Map the IC to the real address
        cout << "secont time address mapped for IC to cheak oprand error" << endl;

        memcpy(IR, M[RA], 4);
        cout << "third time address mapped for IC to regain the IR" << endl;
        // cheak if IR[0] is integer if yes then set PI=2
        if (IR[0] >= '0' && IR[0] <= '9')
        {
        ...
        // Check for any PI interupt as we cheak for invalid page fault above
        if (PI != 0)
        {
            return;
        }
        ...
        // Cheak for any Time Limit Exceeded or Line Limit Exceeded
        if (pcb.TTC > pcb.TTL)
        {
...
int main()
{
    ...
    os.infile.open("input_y.txt", ios::in);
    os.outfile.open("output_y.txt", ios::out);
